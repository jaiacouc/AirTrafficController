...
static void *client_thread(void *arg) {
    ...
    while (myplane->state != PLANE_DONE) {
        // Check for plane in air
        if(myplane->state == PLANE_INAIR){
            printf("Flight %s is in the air -- waiting 4 seconds\n", myplane->id);

            // Check for empty que
            if (que_size()>0){
                printf("Clearning flight %s\n", (char*)que_next(myplane,0));
                sleep(4);
                // Get next planes file stream
                airplane *nextplane = planelist_find((char*)que_next(myplane,0));
                fprintf(nextplane->fp_send, "TAKEOFF\n");
                fflush(nextplane->fp_send);
                myplane->state = 1;
            // If no plane in the que
            }else{
                sleep(4);
